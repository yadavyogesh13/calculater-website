...
function calculatePayoff() {
    ...
    // Calculate minimum payment scenario separately
    const minPaymentAmount = Math.max(minimumPayment, currentBalance * 0.05); // At least 5% of balance
    const minPaymentResult = calculateMinPaymentScenario(currentBalance, annualInterestRate, minPaymentAmount);
    
    // Add comparison data to result
    result.interestSavings = minPaymentResult.totalInterest - result.totalInterest;
    result.timeSavings = minPaymentResult.totalMonths - result.totalMonths;
...
function calculateSingleCardPayoff(balance, annualRate, monthlyPayment) {
    ...
    while (currentBalance > 0 && totalMonths < maxMonths) {
        totalMonths++;
        ...
        // Safety check - if balance isn't decreasing significantly, break
        if (currentBalance >= balance && totalMonths > 1) {
            throw new Error("Payment too small to pay off debt - balance not decreasing");
        }
...
function calculateMinPaymentScenario(balance, annualRate, minPayment) {
    const monthlyRate = annualRate / 100 / 12;
    let currentBalance = balance;
    let totalMonths = 0;
    let totalInterest = 0;
    ...
    while (currentBalance > 0 && totalMonths < maxMonths) {
        ...
        // For minimum payments, ensure we pay at least the interest
        const principalPayment = Math.max(0, Math.min(minPayment - monthlyInterest, currentBalance));
        currentBalance = currentBalance - principalPayment;
            ...
            <div class="space-y-6">
                <div class="bg-red-50 border border-red-200 rounded-xl p-6">
                    <div class="text-center">
                        ...
                        <div class="text-xl font-bold text-red-700 mb-2">Debt May Not Be Paid Off</div>
                        <div class="text-red-600">Your monthly payment might be too low to cover the interest charges.</div>
                        <div class="text-sm text-red-500 mt-2">Try increasing your monthly payment amount.</div>
                    </div>
                </div>
            </div>
        `;
...
function generatePayoffTimeline(balance, annualRate, monthlyPayment) {
    ...
    const monthlyRate = annualRate / 100 / 12;
    let month = 1;
    const maxMonths = 60; // Show max 5 years

    while (currentBalance > 0 && month <= maxMonths) {
        const monthlyInterest = currentBalance * monthlyRate;
        const principalPayment = Math.min(monthlyPayment - monthlyInterest, currentBalance);
        const actualPayment = Math.min(monthlyPayment, currentBalance + monthlyInterest);
        ...
        if (month <= 12 || month % 6 === 0 || currentBalance <= 0) {
            tableHTML += `
                <tr class="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right font-semibold text-blue-600">₹${formatCurrency(actualPayment)}</td>
